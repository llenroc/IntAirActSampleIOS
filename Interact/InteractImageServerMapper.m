...
#import "InteractImageServerMapper.h"
...
- (RKObjectMapping *) serializationMapping {
    if(!_serializationMapping) {
        RKObjectMapping* imageMapping = [RKObjectMapping mappingForClass:[InteractImage class]];
...
- (void)startServer{
    ...
    [httpServer handleMethod:@"GET" withPath:@"/images/:id" block:^(RouteRequest *request, RouteResponse *response) {
        ...
        InteractImage* image = [self.imageServer getImage:number];
